...
(function() {
  nextGenShinyApps::fluidPage(
    ...
    header = nextGenShinyApps::titlePanel(
      ...
      right = nextGenShinyApps::actionButton(
        "closeapp",
        "close",
        style = "pill",
        bg.type = "danger",
    ...
    shiny::div(
      shiny::selectInput(
        "symbolseqnuicopyinsert",
        "Copy or Insert When Clicked?",
        choices = c(
          "Copy to Clipboard" = 1,
          "Insert in Document" = 2
        )
      ),
      shiny::conditionalPanel(condition = "input.symbolseqnuicopyinsert == '2'",
                              shiny::div(
                                shiny::selectInput(
                                  "symbolseqnuiinserttype",
                                  "Insert Type",
                                  choices = c(
                                    "Rmd or Shiny: Insert Code" = "a",
                                    "Rmd: Insert Code with R chunk" = "b",
                                    "Rmd: Insert Shorthand Code" = "c",
                                    "Rmd: Insert Shorthand Code v2" = "d"
                                  )
                                ),
                                class = "pl-2"
                              )),
